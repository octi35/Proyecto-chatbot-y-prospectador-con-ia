  ...
  return (
    <div className="space-y-2">
      ...
      {mode === "text" ? (
        ...
        <div className="space-y-2">
          {products.length === 0 && (
            <div className="text-center py-6 px-4 bg-[#f3f5fb]/70 border border-dashed border-[#e2e5ee] rounded-2xl">
              <ShoppingBag size={22} className="text-[#cbd0e0] mx-auto mb-1.5" />
          ...
          {products.map((p, i) => (
            <div key={i} className="bg-white rounded-2xl p-3 ds-shadow flex gap-3">
              ...
              <div className="shrink-0">
                <div className="w-14 h-14 rounded-xl bg-[#f3f5fb] overflow-hidden flex items-center justify-center">
                  {p.photo ? (
                    <img src={p.photo} alt={p.name} className="w-full h-full object-cover" onError={(e) => (e.currentTarget.style.display = "none")} />
                  ) : (
                    <ImageIcon size={18} className="text-[#cbd0e0]" />
                  )}
                </div>
              </div>
              {/* Fields */}
              <div className="flex-1 min-w-0 space-y-1.5">
                <div className="flex gap-1.5">
                  <input
                    ...
                    className="flex-1 bg-[#f3f5fb] rounded-lg px-2.5 py-1.5 text-[12px] font-semibold text-[#111111] focus:outline-none focus:border-[#4f6ef7]"
                  />
                  <div className="relative w-28 shrink-0">
                    ...
                    <input
                      ...
                      placeholder="Precio"
                      className="w-full bg-[#f3f5fb] rounded-lg pl-5 pr-2 py-1.5 text-[12px] text-[#111111] focus:outline-none focus:border-[#4f6ef7]"
                    />
                  </div>
                </div>
                <input
                  ...
                  className="w-full bg-[#f3f5fb] rounded-lg px-2.5 py-1.5 text-[11px] text-[#6b7280] focus:outline-none focus:border-[#4f6ef7]"
                />
                <input
                  type="url"
                  value={p.photo}
                  ...
                  className="w-full bg-[#f3f5fb] rounded-lg px-2.5 py-1.5 text-[10px] font-mono text-[#6b7280] focus:outline-none focus:border-[#4f6ef7]"
                />
              </div>
              <button
                type="button"
                onClick={() => removeProduct(i)}
                className="p-1.5 text-[#cbd0e0] hover:text-[#e26562] transition-colors cursor-pointer shrink-0 self-start"
          ...
          <button
            type="button"
            onClick={addProduct}
            className="w-full py-2.5 border border-dashed border-[#e2e5ee] rounded-xl text-[12px] font-semibold text-[#4f6ef7] hover:bg-[#f5f6ff]/40 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
          >
            <Plus size={14} /> Agregar producto
          </button>
        </div>
      )}
      <p className="text-[10px] text-[#9ca3af]">Es el núcleo de respuestas del bot. Cuantos más datos, mejor vende.</p>
    </div>
  );
}
