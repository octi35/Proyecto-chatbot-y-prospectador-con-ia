    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="text-center pt-2">
        <motion.div
          ...
          className="inline-flex items-center gap-1.5 text-[12px] font-medium text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full mb-4"
        >
          <BookOpen size={13} /> Centro de ayuda
        </motion.div>
        <h1 className="text-[30px] font-semibold tracking-tight text-zinc-900">¿Cómo funciona Respondo?</h1>
        <p className="text-[15px] text-zinc-500 mt-3 max-w-xl mx-auto leading-relaxed">
          ...
          <Card key={s.n} interactive index={i} className="p-6 flex flex-col" onClick={() => onNavigate(s.tab)}>
            <div className="flex items-center justify-between mb-4">
              <span className={`w-11 h-11 rounded-xl flex items-center justify-center ${
                s.tone === "accent" ? "bg-indigo-50 text-indigo-600" : s.tone === "success" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"
              }`}>{s.icon}</span>
              <span className="text-[28px] font-semibold text-zinc-100 leading-none">{s.n}</span>
            </div>
            <h3 className="text-[16px] font-semibold text-zinc-900">{s.title}</h3>
            <p className="text-[13.5px] text-zinc-500 mt-1.5 leading-relaxed flex-1">{s.desc}</p>
            <span className="inline-flex items-center gap-1 text-[13px] font-medium text-indigo-600 mt-4">
        ...
        <div className="grid sm:grid-cols-2 gap-x-8 gap-y-4">
          {[
            ...
            <div key={i} className="flex gap-3">
              <span className="w-8 h-8 rounded-lg bg-zinc-100 text-zinc-500 flex items-center justify-center shrink-0">{tip.icon}</span>
              <div>
                <p className="text-[13.5px] font-medium text-zinc-900">{tip.t}</p>
                <p className="text-[12.5px] text-zinc-500 mt-0.5">{tip.d}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          {FAQ.map((item, i) => (
            <Card key={i} className="overflow-hidden">
              ...
              >
                ...
                <ChevronDown size={17} className={`text-zinc-400 shrink-0 transition-transform duration-300 ${open === i ? "rotate-180" : ""}`} />
              </button>
              <AnimatePresence initial={false}>
                {open === i && (
                  ...
                  >
                    <p className="px-5 pb-4 text-[13.5px] text-zinc-500 leading-relaxed">{item.a}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </Card>
      ...
      {/* CTA footer */}
      <Card className="p-6 bg-zinc-900 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
        <div>
          <h3 className="text-[16px] font-semibold text-white">¿Listo para empezar?</h3>
          <p className="text-[13px] text-zinc-400 mt-1">Configurá tu agente y empezá a recibir leads hoy.</p>
        </div>
        <button
          onClick={() => onNavigate("playground")}
          className="bg-indigo-600 hover:bg-indigo-500 text-white text-[14px] font-medium px-5 py-2.5 rounded-xl flex items-center gap-2 transition-colors cursor-pointer shrink-0"
        >
