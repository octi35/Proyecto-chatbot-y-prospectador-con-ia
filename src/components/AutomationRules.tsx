    ...
    <div className="bg-white rounded-[22px] p-6 ds-shadow space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-full text-[13px] font-semibold flex items-center gap-1.5 transition-all cursor-pointer ${
            showForm ? "bg-[#f4f4f5] text-[#111111]" : "bg-[#4f6ef7] text-white hover:bg-[#3b5bdb]"
          }`}
        >
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            <div className="bg-[#f3f5fb]/70 rounded-2xl p-4 space-y-4">
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="Nombre de la regla (ej: Avisar leads calientes)"
                className="w-full bg-[#f4f4f5] border border-transparent rounded-xl px-3.5 py-2.5 text-[13px] text-[#111111] focus:outline-none focus:border-[#4f6ef7] transition-colors"
              />
              <div className="grid sm:grid-cols-2 gap-4">
                {/* Trigger */}
                <div className="space-y-2">
                  <label className="text-[11px] font-semibold text-[#a67c00] uppercase tracking-wide flex items-center gap-1">
                    <Zap size={11} /> Cuando pase esto
                  </label>
                  <select
                    value={trigger}
                    onChange={(e) => { setTrigger(e.target.value as AutomationRule["trigger"]); setTriggerValue(""); }}
                    className="w-full bg-[#f4f4f5] border border-transparent rounded-xl px-3 py-2.5 text-[13px] text-[#111111] focus:outline-none focus:border-[#4f6ef7] cursor-pointer"
                  >
                  ...
                  {triggerDef?.needsValue && (
                    <input
                      ...
                      className="w-full bg-[#f4f4f5] border border-transparent rounded-xl px-3 py-2 text-[12px] text-[#111111] focus:outline-none focus:border-[#4f6ef7]"
                    />
                  )}
                </div>
                {/* Action */}
      ...
      {loading ? (
        ...
        <div className="text-center py-10 px-4">
          <div className="w-14 h-14 rounded-2xl bg-[#f3f5fb] flex items-center justify-center mx-auto mb-3">
            <Workflow size={26} className="text-[#cbd0e0]" />
          </div>
          ...
          {rules.map((rule) => (
            <div
              ...
              className={`flex items-center gap-3 p-3.5 rounded-2xl border transition-all ${
                rule.enabled ? "bg-white border-[#f4f4f5] ds-shadow" : "bg-[#f3f5fb]/60 border-[#f4f4f5] opacity-70"
              }`}
            >
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${rule.enabled ? "bg-[#fff7e0] text-[#b8860b]" : "bg-[#f4f4f5] text-[#9ca3af]"}`}>
                <Zap size={16} />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-[#6b7280] mt-0.5 flex-wrap">
                  ...
                  <span className="bg-[#f5f6ff] text-[#4f6ef7] px-1.5 py-0.5 rounded-md font-medium">{actionLabel(rule.action)}{rule.actionValue ? `: ${rule.actionValue}` : ""}</span>
                  {(rule.timesTriggered ?? 0) > 0 && (
                    <span className="text-[10px] text-[#9ca3af]">· {rule.timesTriggered}× ejecutada</span>
                  )}
                </div>
              </div>
              {/* iOS toggle */}
              <button
                ...
                className={`relative w-10 h-[22px] rounded-full transition-colors duration-300 shrink-0 cursor-pointer ${rule.enabled ? "bg-[#7dd87d]" : "bg-[#cbd0e0]"}`}
              >
              ...
              <button
                ...
                className="p-1.5 text-[#cbd0e0] hover:text-[#e26562] transition-colors cursor-pointer shrink-0"
